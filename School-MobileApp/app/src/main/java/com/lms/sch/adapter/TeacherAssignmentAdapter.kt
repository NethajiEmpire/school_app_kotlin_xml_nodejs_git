...
class TeacherAssignmentAdapter (
    ...
    val isDiary : Boolean,
    val list : ArrayList<GetTeacherAssignmentResponse.Result.Rows>,
    ...
    inner class  ViewHolder(view: View, type: Int) : RecyclerView.ViewHolder(view){
        var binding1: CardAssignmentBinding?= null
        var binding: CardAssignment1Binding?= null
        init {
            if (type == 0){
                binding1 = CardAssignmentBinding.bind(view)
            }
            else {
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        if (isDiary){
            ...
            if (list[position].createdBy != null){
                holder.binding1!!.teacherName.text = "${list[position]. createdBy.firstName?: ""} ${list[position].createdBy!!.lastName ?: ""}"
            }
            else {
                holder.binding1!!.teacherName.text = "--/--"
            }*/
            if (list[position].title != null){
                holder.binding1!!.assignmentTitle.text = list[position].title
            }
            else {
            ...
            if (list[position].attachment != null && list[position].attachment!!.isNotEmpty()){
                holder.binding1!!.attach.text = "+${list[position].attachment!!}"
         ...
         /*   holder.binding1!!.attachLay.setOnClickListener {
                if (list[position].attachment != null && list[position].attachment!!.isNotEmpty()){
                    if (list[position].attachment!![0].endsWith(".pdf")){
                        val bundle = Bundle()
                        bundle.putString(Constants.IntentKeys.KEY,
                            list[position].attachment!![0].toString()
                        )
                        ...
                        intent.setData(Uri.parse(list[position].attachment!![0].toString()))
            ...
            when(list[position].status){
                ...
                "completed" ->{
                    ...
                    if (list[position].updatedAt != null){
                        holder.binding1!!.assUpdatedDate.text = BaseUtils.getFormattedDate(list[position].updatedAt!!,Constants.ApiKeys.TIME_INPUT_FORMAT,Constants.ApiKeys.DATE_FORMAT)
                    }
                    else{
                        holder.binding1!!.assUpdatedDate.text = "--/--"
                    }
            ...
            holder.binding!!.viewDoc.setOnClickListener {
                if (list[position].attachment != null && list[position].attachment!!.isNotEmpty()){
                    ...
                    else if (list[position].attachment!!.endsWith(".png") ||
                        list[position].attachment!!.endsWith(".jpeg") ||
                        list[position].attachment!!.endsWith(".jpg")) {

                        val doc = ArrayList<String>()

                        val attachments = list[position].attachment!!.split(",")

                        for (item in attachments) {
                            if (item.trim().endsWith(".jpg") || item.trim().endsWith(".jpeg") || item.trim().endsWith(".png")) {
                                doc.add(item.trim())
                            }
                        }
...
//                        BaseUtils.startActivity(mActivity, ImageViewActivity(),bundle,false)
            ...
            holder.binding!!.root.setOnClickListener {
                val intent = Intent(mActivity, AssignmentActivity::class.java)
...
/*  if(list[position].subject != null && list[position].subject!!.name != null){
  ...
  if(list[position].studentClass != null && list[position].studentClass!!.name != null){
      holder.binding.teacherName.text = list[position].studentClass!!.name
  }else{
  ...
  holder.binding!!.root.setOnClickListener {
      if(list[position].status == "completed"){
          ...
          BaseUtils.startActivity(mActivity, AssignmentActivity(), bundle, false)
      }else{
          UiUtils.showSnack("This is a ongoing assignment", holder.binding.root,false)
      }
  }
  when(list[position].status){
      "pending" ->{
